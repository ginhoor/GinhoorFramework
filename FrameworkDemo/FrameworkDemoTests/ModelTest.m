...
- (void)testPeopleModel
{
    ...
    People *a = [People data:[string jsonDictionary]];
...
- (void)testJsonAPI
{
    ...
    [[aString jsonArray] enumerateObjectsUsingBlock:^(NSDictionary *json, NSUInteger idx, BOOL *stop) {
        People *a = [People data:json];
        XCTAssertNotNil(a, @"model is nil");
        XCTAssertNotNil(a.name, @"person has not a name!");
        XCTAssertNotNil(a.avatar, @"person has not a avatar!");
